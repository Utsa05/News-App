...
class NewsFeed extends StatefulWidget {
  const NewsFeed({Key? key, required this.categoryName}) : super(key: key);
...
class _NewsFeedState extends State<NewsFeed> {
  ...
  void initState() {
    widget.categoryName.isEmpty
        ? newsCubit.loadNews()
        : newsCubit.loadbycategoryNews(widget.categoryName);
  ...
  Widget build(BuildContext context) {
    return widget.categoryName.isEmpty
        ...
        : BlocBuilder<NewsCubit, NewsState>(
            builder: (context, state) {
              if (state is NewsbyCategoryLoaded) {
                final List<NewsEntity> news = state.news;
                return ListView.builder(
                    physics: const BouncingScrollPhysics(),
                    padding: const EdgeInsets.symmetric(
...
class NewsItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: (() => Navigator.pushNamed(context, AppRouteConts.newsView,
          arguments: newsEntity.url)),
      child: Container(
